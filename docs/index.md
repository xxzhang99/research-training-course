---
hide:
  - navigation
  - toc
---

<div class="course-hero simple-hero">
<div>
<div class="course-eyebrow">Wuhan University · Computer Science</div>
<h1>WHU CS<br>科研训练</h1>
<p class="lead">面向计算机及智能信息类本科生的科研素养与研究实践课程。课程围绕一个规模受控的研究题目，训练学生从文献出发，经过问题、机理、解法、实现和实验，形成可以复查与修订的研究提案。</p>
<div class="hero-actions">
<a class="primary" href="schedule/">课程日历</a>
<a href="assignments/">课程作业</a>
<a href="faq/">常见问题</a>
</div>
</div>
<div class="course-facts">
<strong>10讲</strong><span>科研方法与实践</span>
<strong>公开资料</strong><span>PDF与可编辑PPTX</span>
<strong>贯穿项目</strong><span>Mini Research Proposal</span>
</div>
</div>

<section class="home-section">
<h2>课程简介</h2>
<p>本课程不把文献阅读、方法构思、实验设计和论文写作拆成互不相关的技巧，而是让每个小组围绕同一个研究问题连续推进。课程强调问题的重要性与边界、机理和假设、方案可行性、证据质量、学术诚信，以及对失败和局限的诚实分析。</p>
</section>

<section class="home-section">
<h2>课程信息</h2>
<div class="course-info-list">
<div><strong>授课教师</strong><span>晏潇 博士</span></div>
<div><strong>单位</strong><span>武汉大学 · 武汉数学与智能研究院</span></div>
<div><strong>课程形式</strong><span>方法讲授、案例拆解、课堂练习、贯穿式研究项目与同伴反馈</span></div>
<div><strong>课程对象</strong><span>大三及以上计算机与智能信息相关专业学生</span></div>
<div><strong>上课日期</strong><span>待定</span></div>
<div><strong>联系邮箱</strong><span>yanxiaosunny@whu.edu.cn</span></div>
</div>
</section>

<section class="home-section">
<h2>课程安排</h2>
<ol class="course-outline">
<li><strong>科研基础与文献能力</strong><span>科研范式、论文阅读、文献调研和研究评价。</span></li>
<li><strong>问题、方案与研究设计</strong><span>问题与机理、研究假设、方案构思、实现、实验与证据。</span></li>
<li><strong>写作、交流与提案</strong><span>论文写作、图表、AI与科研诚信、答辩和研究路径。</span></li>
</ol>
</section>

<section class="home-section">
<h2>当前课件</h2>
<table class="simple-course-table">
<thead><tr><th>讲次</th><th>主题</th><th>PDF</th><th>PPTX</th><th>页面</th></tr></thead>
<tbody>
<tr><td>02</td><td>科研文献阅读</td><td><a href="assets/files/lecture-02/slides.pdf">下载</a></td><td><a href="assets/files/lecture-02/slides.pptx">下载</a></td><td><a href="lectures/lecture-02/">查看</a></td></tr>
<tr><td>03</td><td>科研方法</td><td><a href="assets/files/lecture-03/slides.pdf">下载</a></td><td><a href="assets/files/lecture-03/slides.pptx">下载</a></td><td><a href="lectures/lecture-03/">查看</a></td></tr>
</tbody>
</table>
</section>

<section class="home-section home-note">
<h2>资料说明</h2>
<p>公开PPTX为课程完整版，包含授课备注、课堂任务和参考答案。外部论文原则上只提供官方链接。课程日期、截止节点和后续讲次将在信息确定后统一更新。</p>
</section>
